...
DT_PAGRINDINE_DIR = 'DT_eksperimentai' # Aplankas medžiams
...
# ---------- 1. ĮKELIAME DUOMENIS ----------
print("=" * 80)
print(" 1. DUOMENŲ ĮKĖLIMAS (DECISION TREE) ".center(80, "="))
print("=" * 80)
...
try:
    ...
    print(f"KLAIDA: Nerastas '{JSON_FAILAS_PATH}'.")
    exit()
...
for eksp_pavadinimas, pozymiai in eksperimentai.items():
    ...
    try:
        ...
        print(f"  [!] KLAIDA: Požymis {e} nerastas.")
        continue
    ...
    for d in range(1, 21):
        ...
        tuning_data.append({'depth': d, 'f1': f1_val, 'accuracy': acc_val})
    ...
    plt.title(f'Gylio parinkimas: {eksp_pavadinimas}')
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Oranges', cbar=False) # Oranges spalva
...
plt.title('Decision Tree: F1 balų palyginimas')
plt.xlabel('F1 Score')
plt.xlim(0, 1.05)
plt.tight_layout()
...
if roc_duomenys_bendrai:
    ...
    plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
    plt.title('Visų eksperimentų ROC (Decision Tree)')
...
print(f"\nVisi DT rezultatai išsaugoti: {BASE_OUTPUT_DIR}")