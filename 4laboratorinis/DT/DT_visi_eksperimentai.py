...
DT_PAGRINDINE_DIR = 'DecisionTree_eksperimentai'
JSON_DIREKTORIJA = '../JSON'
JSON_FAILAS = 'pozymiu_rinkiniai.json'

JSON_FAILAS_PATH = os.path.join(JSON_DIREKTORIJA, JSON_FAILAS)
BASE_OUTPUT_DIR = os.path.join(GRAFIKU_DIREKTORIJA, DT_PAGRINDINE_DIR)
os.makedirs(BASE_OUTPUT_DIR, exist_ok=True)

# ---------- 1. ĮKELIAME DUOMENIS ----------
print("=" * 80)
print(" 1. DUOMENŲ ĮKĖLIMAS (DECISION TREE VISI EKSPERIMENTAI) ".center(80, "="))
print("=" * 80)
...
for eksp_pavadinimas, pozymiai in eksperimentai.items():
    ...
    # --- 3.3. Hiperparametrų parinkimas (DT: max_depth) ---
    best_depth = -1
    best_val_f1 = -1
    tuning_data = []
    ...
    for depth in range(1, 21):
        ...
        tuning_data.append({'depth': depth, 'f1': f1_val, 'accuracy': acc_val})
    ...
    plt.plot(td_df['depth'], td_df['f1'], marker='o', label='F1 Score')
    plt.plot(td_df['depth'], td_df['accuracy'], marker='s', linestyle='--', label='Accuracy')
    ...
    visu_eksperimentu_rezultatai.append([
        eksp_pavadinimas, len(pozymiai), best_depth, acc, prec, rec, f1
    ])
    ...
    # --- 3.7. ROC ir AUC ---
    if hasattr(final_model, "predict_proba") and 2 in final_model.classes_:
        class_index = list(final_model.classes_).index(2)
        y_proba = final_model.predict_proba(X_test)[:, class_index]
...
df_rezultatai = pd.DataFrame(
    visu_eksperimentu_rezultatai,
    columns=['Eksperimentas', 'Požymių sk.', 'Best Depth', 'Accuracy', 'Precision', 'Recall', 'F1 Score']
)
df_rezultatai = df_rezultatai.sort_values(by='F1 Score', ascending=False).reset_index(drop=True)

print("\n" + "=" * 100)
print(" DT GALUTINIAI REZULTATAI (VISI EKSPERIMENTAI) ".center(100, "="))
print("=" * 100)
...
plt.title('Decision Tree: F1 balų palyginimas (Visi požymių rinkiniai)')
plt.xlabel('F1 Score')
plt.xlim(0, 1.05)
...
if roc_duomenys_bendrai:
    ...
    plt.title('Visų eksperimentų ROC kreivių palyginimas (Decision Tree)')
...
# ---------- 5. GERIAUSIŲ REZULTATŲ SUVESTINĖ ----------
print("\n" + "=" * 80)
...
print(f"Didžiausias ACCURACY:   {row_acc['Eksperimentas']:<25} (Reikšmė: {row_acc['Accuracy']:.4f})")
print(f"Didžiausias PRECISION:  {row_prec['Eksperimentas']:<25} (Reikšmė: {row_prec['Precision']:.4f})")
print(f"Didžiausias RECALL:     {row_rec['Eksperimentas']:<25} (Reikšmė: {row_rec['Recall']:.4f})")
print(f"Didžiausias F1 SCORE:   {row_f1['Eksperimentas']:<25} (Reikšmė: {row_f1['F1 Score']:.4f})")
